...
until Country.count == 20 do
  ...
  Article.create(title: Faker::Creature::Cat.name, description: Faker::Creature::Cat.breed, when_went:  Faker::Date.between(from: 10.years.ago, to: Date.today), country_id:  countries.sample.id)
  end
  ...
  until Comment.count == 200 do
      ...
      kinds = Article::Kinds
      until Reaction.count == 1000 do rel_type = r_type.sample
      if rel_type == "article"
